...
public class MainViewModel : ObservableObject
{
    ...
    private DateTimeOffset? unlockUntil;
    ...
    {
        get => config.UnlockMinutes;
        set
        {
            var clamped = Math.Clamp(value, 1, 240);
            if (config.UnlockMinutes != clamped)
            {
                config.UnlockMinutes = clamped;
                OnPropertyChanged();
            }
        }
    }
    ...
    {
        new("toggle-protection", "\uC7A0\uAE08 \uC801\uC6A9", "\uC784\uC2DC \uD574\uC81C \uC0C1\uD0DC\uB97C \uB05D\uB0B4\uACE0 \uC571 \uBAA9\uB85D\uC758 \uC7A0\uAE08\uC744 \uB2E4\uC2DC \uC801\uC6A9\uD569\uB2C8\uB2E4.", "Ctrl+Alt+P"),
        new("temporary-unlock", "\uC784\uC2DC \uD574\uC81C", "\uC124\uC815\uB41C \uC2DC\uAC04 \uB3D9\uC548 \uC7A0\uAE08\uC744 \uD574\uC81C\uD569\uB2C8\uB2E4.", "Ctrl+Alt+U"),
        new("lock-now", "\uC989\uC2DC \uC7A0\uAE08", "\uD604\uC7AC \uD574\uC81C \uC0C1\uD0DC\uB97C \uB05D\uB0B4\uACE0 \uB2E4\uC2DC \uC7A0\uAE09\uB2C8\uB2E4.", "Ctrl+Alt+L"),
    ...
    public async Task InitializeAsync()
    {
        ...
        config.UnlockUntil = loaded.UnlockUntil;
        unlockUntil = ParseUnlockUntil(config.UnlockUntil);
        OnPropertyChanged(nameof(CloseToBackground));
        OnPropertyChanged(nameof(UnlockMinutes));
    ...
    {
        if (!enabled && config.ProtectedApps.Any(app => app.Enabled))
        {
            ProtectionEnabled = LockRulesActive();
            StatusText = "\uC7A0\uAE08\uB41C \uC571\uC774 \uC788\uC5B4 \uBCF4\uD638\uB97C \uB044\uB294 \uBC29\uC2DD\uC73C\uB85C\uB294 \uD574\uC81C\uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4. \uC784\uC2DC \uD574\uC81C\uB97C \uC0AC\uC6A9\uD558\uC138\uC694.";
            await SaveAsync();
            return;
        }
    ...
    public async Task SavePreferencesAsync(bool closeToBackground, int unlockMinutes, int authCacheMinutes)
    {
    ...
    public async Task TemporaryUnlockAsync()
    {
        unlockUntil = DateTimeOffset.Now.AddMinutes(Math.Clamp(config.UnlockMinutes, 1, 240));
        config.UnlockUntil = unlockUntil.Value.ToString("O");
        await SyncProtectionRulesAsync();
        await SaveAsync();
        UpdateStatus();
    }
    ...
    private async Task SyncProtectionRulesAsync()
    {
        try
        {
            PruneExpiredUnlockSession();

            var apps = !UnlockSessionActive()
                ? config.ProtectedApps.Where(app => app.Enabled)
                : Enumerable.Empty<ProtectedApp>();
    ...
    private async Task SaveAsync()
    {
        ...
        config.UnlockUntil = unlockUntil?.ToString("O");
    ...
    private bool UnlockSessionActive()
    {
        PruneExpiredUnlockSession();
        return unlockUntil is not null && DateTimeOffset.Now < unlockUntil;
    }

    private bool LockRulesActive()
    {
        return config.ProtectedApps.Any(app => app.Enabled) && !UnlockSessionActive();
    }

    private void PruneExpiredUnlockSession()
    {
        if (unlockUntil is not null && DateTimeOffset.Now >= unlockUntil)
        {
            unlockUntil = null;
            config.UnlockUntil = null;
        }
    ...
    private static DateTimeOffset? ParseUnlockUntil(string? value)
    {
    ...
    {
        if (UnlockSessionActive() && unlockUntil is not null)
        {
            StatusText = $"\uC7A0\uAE08 \uC0C1\uD0DC: {unlockUntil:HH:mm}\uAE4C\uC9C0 \uC784\uC2DC \uD574\uC81C";
            return;
        }
