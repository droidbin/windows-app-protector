...
internal sealed class ProtectionWorker : IDisposable
{
    ...
    private void Tick()
    {
        ...
        try
        {
            ...
            if (UnlockSessionActive(config.UnlockUntil))
            {
                RemoveManagedRules();
                return;
            }
    ...
    private static bool UnlockSessionActive(string unlockUntil)
    {
        DateTimeOffset parsed;
        return !string.IsNullOrWhiteSpace(unlockUntil)
            && DateTimeOffset.TryParse(unlockUntil, out parsed)
            && DateTimeOffset.Now < parsed;
    }
